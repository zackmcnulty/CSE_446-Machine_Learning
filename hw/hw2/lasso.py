...
current_lam = lam_max
lam_vals = []
delta = 10e-4 # threshold used to determine when to stop searching for optimal w
prev_w = None
...
while current_lam >= 0.01:
    ...
    (w, b) = lasso_coordinate_descent(X, y, current_lam, prev_w, delta)
...
i1 = np.where(df_train.columns == "agePct12t29") # find columns where each of these vars reside
i2 = np.where(df_train.columns == "pctWSocSec")
i3 = np.where(df_train.columns == "pctUrban")
i4 = np.where(df_train.columns == "agePct65up")
i5 = np.where(df_train.columns == "householdsize")
...
y_pred_train = np.dot(W.T, X.T) + np.expand_dims(B, axis=1)
SSE_train = np.sum(np.square(y_pred_train - y.T), axis=1)
y_pred_test = np.dot(W.T, X_test.T) + np.expand_dims(B, axis=1)
SSE_test = np.sum(np.square(y_pred_test - y_test.T), axis=1)
